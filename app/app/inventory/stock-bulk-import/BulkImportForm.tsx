...
import { bulkImportStockItems, type StockBulkImportResult } from "../stock-bulk-import-actions";
...
export default function BulkImportForm() {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await bulkImportStockItems(fd);
      setResult(res);
      if (res.createdCount) {
        setFileName(null);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 8, padding: "14px 16px" }}>
        ...
        <div style={{ fontSize: 12.5, color: "var(--muted)", marginBottom: 10 }}>
          An Excel file with every field a stock item supports (name, type, code, cost price, sell price, opening quantity) and one example row.
        </div>
        <a
          href="/api/inventory/stock-import-template"
      ...
      {result?.createdCount != null && (
        <div style={{ fontSize: 13, fontWeight: 600, color: "var(--good)", background: "var(--good-tint)", border: "1px solid var(--good-border, var(--line))", borderRadius: 8, padding: "10px 13px" }}>
          Imported {result.createdCount} stock item{result.createdCount === 1 ? "" : "s"} successfully.
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        >
          {pending ? "Importing…" : "Import stock items"}
        </button>
        <Link href="/app/inventory?tab=stock" style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 8, padding: "9px 18px", fontSize: 13.5, fontWeight: 600, textDecoration: "none", color: "var(--ink)" }}>
          Back to Stock
        </Link>
      </div>
    </div>
