...
async function SchoolDetail({ school, studentCount }: { school: Awaited<ReturnType<typeof db.school.findMany>>[number] & { users: { name: string; email: string }[] }; studentCount: number }) {
  const admin = school.users[0];
  const style = STATUS_STYLE[school.status];
  const staffCount = await db.staffProfile.count({ where: { schoolId: school.id } });
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", marginTop: 14 }}>
        <div style={{ background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 8, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <DetailRow label="Admin email" value={admin?.email ?? "—"} mono />
      ...
      <Link href={`/super-admin/subscriptions?school=${school.id}`} style={{ marginTop: 12, textAlign: "center", background: "var(--card)", border: "1px solid var(--line)", borderRadius: 8, padding: 9, fontSize: 12.5, fontWeight: 600, color: "var(--ink)", textDecoration: "none" }}>
        View billing →
      </Link>
    </div>
  );
